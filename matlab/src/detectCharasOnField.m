...
for i = 1:6
    ...
    if isCharaDead(charaIcon, bgimage, c2, charaIconsArr);
        charaOnField{i,1} = "dead";
        charaOnField{i,2} = judgeGray(c2);
    else
        charaOnField{i, 1} = charaNamesArr{c2};
        charaOnField{i,2} = judgeGray(c2);
    end
...
for i = 1:6
    ...
    [r2,c2,v] = find(judgeGray == max(judgeGray));
    if isCharaDead(charaIcon, bgimage, c2, charaIconsArr);
        charaOnField{i+6, 1} = "dead";
        charaOnField{i+6,2} = judgeGray(c2);
    else
        charaOnField{i+6, 1} = charaNamesArr{c2};
        charaOnField{i+6,2} = judgeGray(c2);
    end
end
